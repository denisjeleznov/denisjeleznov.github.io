...
Является "легковесной" альтернативой (не эквивалентной заменой) классическим (2PC) распределённым транзакциям. Паттерн похож на "оркестрационную" сагу, но формально ею не является ибо в данном случае оркестратор управляет связанными ресурсами а не самостоятельными системами.

1PC pattern предполагает что:
1. имеется "оркестратор", который последовательно запускает операции над списком ресурсов
2. операции, выполняемые над ресурсами, идемпотентны
...
На практике, транзакции в реляционных БД выполняются на уровнях изоляции repeatable read или read committed. При этом, часто разработчики не отдают себе отчёт в том, что использование этих уровней изоляции **не гарантирует защиту от потери обновлений**. 

Дело в том, что транзакция, как правило, реализует логику "read-then-update" - для этой логики только уровень изоляции serializable может гарантировать отсутствие потери обновлений (repeatable read должен откатывать "протухшее" изменение, однако это работает не для всех СУБД - например, не работает на MySQL).

Для корректной реализации логики "read-then-update" можно использовать следующие варианты:
...
Так же - для предотвращения зваимоного блокирования, важно соблюдать порядок получения (и возврата) блокировок. Блокировки должны получаться упорядоченно - должен быть определён порядок их получения. Возврат блокировок должен происходить в порядке обратном получению.
