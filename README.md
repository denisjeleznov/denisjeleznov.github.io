...
В классе String есть intern(), который помещает строку в JVM string pool и позволяет получить ссылку на "каноническое представление строки". На практике этот метод используется редко ибо современные JVM достаточно умеют эффективно хранить стороки - нет необходимости инициировать интернирование строк внучную.
