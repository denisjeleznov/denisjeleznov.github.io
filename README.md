...
Одна из важнейших задач Архитектора - разделить будущую систему на компоненты и определить границы между ними. Для этого можно применить аналогию из физики - сильное взаимодействие  (обеспечивает устойчивость атомных ядер). В мире ИТ аналогом сильного взаимодействия можно считать бизнес-транзакции. Соответственно, границы между компонентами логично провести по границе бизнес-транзакций (транзакция должна полностью "помещаться" внутри одного компонента.
...
Для решения проблемы координации измений нужно поддерживать слабую связность между компонентами системы. Достичь этого можно двумя подходами:
