...
* компопент это сбособ разделения функционала

В этой классификации, библиотека кода (представляет собой код, предназначенный для повторного использования) относится к модулю (не к компоненту).
